...
@section('content')
<div class="max-w-2xl">
    <form action="{{ route('pegawai.store') }}" method="POST" class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 space-y-4">
        @csrf

        <div>
            <label class="block text-sm mb-1">Nama</label>
            <input name="nama" value="{{ old('nama') }}" class="w-full px-3 py-2 border rounded dark:bg-gray-900 dark:border-gray-700" required />
            @error('nama')<div class="text-red-600 text-xs mt-1">{{ $message }}</div>@enderror
        </div>

        <div>
            <label class="block text-sm mb-1">Username</label>
            <input name="username" value="{{ old('username') }}" class="w-full px-3 py-2 border rounded dark:bg-gray-900 dark:border-gray-700" required />
        ...
        <div>
            ...
            <input type="email" name="email" value="{{ old('email') }}" class="w-full px-3 py-2 border rounded dark:bg-gray-900 dark:border-gray-700" />
        ...
        <div>
            ...
            <input type="password" name="password" class="w-full px-3 py-2 border rounded dark:bg-gray-900 dark:border-gray-700" required minlength="6" />
        ...
        <div>
            ...
            <input type="text" name="no_telp" value="{{ old('no_telp') }}" class="w-full px-3 py-2 border rounded dark:bg-gray-900 dark:border-gray-700" />
        ...
        <div id="cabangs-section">
            ...
            <div class="space-y-2 max-h-48 overflow-y-auto border rounded p-2 dark:bg-gray-900 dark:border-gray-700">
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="cabangs[]" value="{{ $cabang->id_cabang }}" class="rounded" />
